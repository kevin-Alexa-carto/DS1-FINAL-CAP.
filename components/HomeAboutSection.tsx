...
import { User, ShieldCheck, Smartphone, Globe, Clock, Lock, Landmark, BarChart3 } from 'lucide-react';
...
export const HomeAboutSection: React.FC = () => {
  ...
  return (
    <section className="w-full py-20 lg:py-28 px-4 bg-[#F4F2EB] dark:bg-brand-dark-bg transition-colors duration-500 overflow-hidden relative">
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
        ...
        <div className="flex flex-col gap-8 relative z-10 order-2 lg:order-1">
          ...
          <RevealOnScroll>
            <div className="inline-block bg-brand-green px-4 py-1.5 rounded-full mb-4 shadow-sm">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mt-2">
            <RevealOnScroll delay={200} className="h-full">
              <div className="bg-[#cdfae8] dark:bg-white/5 p-6 rounded-[1.8rem] h-full border border-brand-green/20 shadow-sm hover:shadow-md transition-shadow group hover:-translate-y-1 duration-300">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-10 h-10 bg-white/60 dark:bg-brand-green/20 rounded-full flex items-center justify-center text-emerald-700 dark:text-brand-green group-hover:scale-110 transition-transform">
                     <User className="w-5 h-5" strokeWidth={2.5} />
                  </div>
                  <h3 className="text-slate-900 dark:text-white font-extrabold text-lg leading-tight">Servicio Personalizado</h3>
                </div>
                <p className="text-sm text-slate-800 dark:text-slate-300 font-semibold leading-relaxed opacity-90">
        ...
        {/* --- COLUMNA DERECHA: Animación Circular --- */}
